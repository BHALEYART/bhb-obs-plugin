...
// Only runs in packaged app (not during npm start dev mode).
let autoUpdater = null;
if (app.isPackaged) {
  try {
    autoUpdater = require('electron-updater').autoUpdater;
    autoUpdater.autoDownload    = true;   // download silently in background
    autoUpdater.autoInstallOnAppQuit = false; // we'll prompt the user first
    autoUpdater.logger = null;            // suppress console noise
  } catch (e) {
    // electron-updater not available — skip silently
    autoUpdater = null;
  }
}
...
function setupAutoUpdater() {
  ...
  autoUpdater.on('update-available', (info) => {
    if (controlWin && !controlWin.isDestroyed()) {
      controlWin.webContents.send('update-available', {
  ...
  autoUpdater.on('update-downloaded', (info) => {
    if (controlWin && !controlWin.isDestroyed()) {
      controlWin.webContents.send('update-downloaded', {
        version: info.version
      });
    }
  });
  ...
  autoUpdater.on('error', () => {
    // Silent — update errors are non-critical
  });
...
// IPC: user clicked "Check for Updates" manually
